...
async def on_voice_state_update(MEMBER, before: discord.VoiceState, after: discord.VoiceState) -> None:
    check_time = datetime.datetime.now(datetime.timezone.utc).time()
    DotW = datetime.datetime.now().weekday()
    ...
    if (DotW < 5) and ((check_time < sBreak) or (eBreak < check_time < cutoff)):
